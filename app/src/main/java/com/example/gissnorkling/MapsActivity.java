package com.example.gissnorkling;
...
public class  MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private void getlokasi(){
        String url="https://gisuas.000webhostapp.com/snorkling.php";
            ...
            public void onResponse(JSONArray response) {
                ...
                for (int i = 0; i < jumData; i++) {
                    try {
                        ...
                        MarkerD[i] = false;
                        mMap.addMarker(new MarkerOptions().position(latLng[i]).snippet(telepon[i]).icon(BitmapDescriptorFactory.fromResource(R.drawable.iconn)).title(nama[i]));
                        ...
                        for (int i = 0; i < jumData; i++) {
                            if (marker.getTitle().equals(nama[i])) {
                                if (MarkerD[i]) {
                                    ...
                                    Toast pesan = Toast.makeText(MapsActivity.this, "silahkan klik sekalilagi untuk detail", Toast.LENGTH_LONG);
